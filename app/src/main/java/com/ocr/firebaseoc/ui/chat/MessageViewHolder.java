...
import com.ocr.firebaseoc.ui.models.Message;
...
public class MessageViewHolder extends RecyclerView.ViewHolder {
    ...
    public void updateWithMessage(Message message, RequestManager glide){
        ...
        binding.messageTextView.setTextAlignment(isSender ? View.TEXT_ALIGNMENT_TEXT_END
                : View.TEXT_ALIGNMENT_TEXT_START);

        // Update date
        if (message.getDateCreated() != null) binding.dateTextView
                .setText(this.convertDateToHour(message.getDateCreated()));

        // Update isMentor
        binding.profileIsMentor.setVisibility(message.getUserSender()
                .getIsMentor() ? View.VISIBLE : View.INVISIBLE);
    ...
    private void updateLayoutFromSenderType(){
        ...
        ((GradientDrawable) binding.messageTextContainer
                .getBackground()).setColor(isSender ? colorCurrentUser : colorRemoteUser);
    ...
    private void updateMessageContainer(){
        ...
        // Update the constraint (gravity) for the text of the message
        // (content + date) (Align it to the left for receiver message)
